// Binary Optimal Subtree between roots i and j.
...
pair< vector< vector<long long int> >, vector< vector<int> > > optimalSubtree(int A[], int n){
...
int main(){
	...
	auto ret = optimalSubtree(A, n);

	vector< vector<long long int> > M= ret.first;
	vector< vector<int> > S= ret.second;

	cout << "Min number of divisions are: " << M[1][n-1] << endl;

	cout << "Optimal Subtree ordering:" << endl;
	print(S, 0, n-1);
	cout << endl;
